<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Admin Dashboard - RENTramuros</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
    <link rel="stylesheet" href="css/admin.css">
</head>
<body class="flex h-screen bg-gray-50 antialiased overflow-hidden">

    <!-- HOVERABLE SIDEBAR -->
    <aside class="relative h-screen bg-gray-800 text-white flex flex-col shadow-2xl transition-all duration-300 w-20 hover:w-64 overflow-hidden group hidden md:flex shrink-0 z-50">
        
        <!-- Logo Area -->
        <div class="flex items-center h-20 px-4 border-b border-gray-700 whitespace-nowrap">
            <div class="w-12 flex justify-center shrink-0">
                <i class="fas fa-chess-rook text-3xl text-gray-300"></i>
            </div>
            <div class="ml-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-100">
                <div class="font-bold text-xl tracking-wide">RENT<span class="font-light">ramuros</span></div>
                <div class="text-[10px] text-gray-400 uppercase tracking-wider font-semibold">Admin Hub</div>
            </div>
        </div>

        <!-- Navigation Links -->
        <nav class="flex-1 px-4 py-6 space-y-3 overflow-y-auto overflow-x-hidden">
            
            <a href="admin.php" class="flex items-center py-3 bg-[#7a3229] text-white rounded-lg shadow-md transition whitespace-nowrap">
                <div class="w-12 flex justify-center shrink-0"><i class="fas fa-home text-xl"></i></div>
                <span class="ml-3 font-bold opacity-0 group-hover:opacity-100 transition-opacity duration-300">Home Page</span>
            </a>
            
            <a href="manage_guides.php" class="flex items-center py-3 text-gray-300 rounded-lg hover:bg-gray-700 hover:text-white transition whitespace-nowrap">
                <div class="w-12 flex justify-center shrink-0"><i class="fas fa-users text-xl"></i></div>
                <span class="ml-3 font-medium opacity-0 group-hover:opacity-100 transition-opacity duration-300">Guides</span>
            </a>
            
            <a href="manage_events.php" class="flex items-center py-3 text-gray-300 rounded-lg hover:bg-gray-700 hover:text-white transition whitespace-nowrap">
                <div class="w-12 flex justify-center shrink-0"><i class="fas fa-calendar-alt text-xl"></i></div>
                <span class="ml-3 font-medium opacity-0 group-hover:opacity-100 transition-opacity duration-300">Events</span>
            </a>
            
            <a href="manage_attractions.php" class="flex items-center py-3 text-gray-300 rounded-lg hover:bg-gray-700 hover:text-white transition whitespace-nowrap">
                <div class="w-12 flex justify-center shrink-0"><i class="fas fa-archway text-xl"></i></div>
                <span class="ml-3 font-medium opacity-0 group-hover:opacity-100 transition-opacity duration-300">Attractions</span>
            </a>
            
            <a href="statistics_overview.php" class="flex items-center py-3 text-gray-300 rounded-lg hover:bg-gray-700 hover:text-white transition whitespace-nowrap">
                <div class="w-12 flex justify-center shrink-0"><i class="fas fa-chart-pie text-xl"></i></div>
                <span class="ml-3 font-medium opacity-0 group-hover:opacity-100 transition-opacity duration-300">Statistics</span>
            </a>
        </nav>
    </aside>

    <!-- MAIN CONTENT WRAPPER -->
    <div class="flex-1 flex flex-col h-screen overflow-hidden relative bg-gray-50">
        
        <!-- Minimal Top Header -->
        <header class="bg-white shadow-sm py-4 px-6 flex justify-between items-center z-10 border-b border-gray-200 shrink-0">
            <button class="md:hidden text-gray-600 focus:outline-none hover:text-[#7a3229]">
                <i class="fas fa-bars text-xl"></i>
            </button>
            
            <div class="hidden md:block"></div>

            <!-- User Controls -->
            <div class="flex items-center space-x-6">
                <div id="current-time" class="text-gray-500 text-sm font-mono hidden md:block"></div>
                <div class="text-sm font-semibold text-gray-700 border-l pl-6 border-gray-300">
                    Welcome, Admin
                </div>
                <a href="../../pages/login_page/login.php" class="text-red-500 hover:text-red-700 font-bold transition flex items-center text-sm">
                    <i class="fas fa-sign-out-alt mr-2"></i> Logout
                </a>
            </div>
        </header>

        <!-- Scrollable Main Content Area -->
        <main class="flex-1 overflow-x-hidden overflow-y-auto p-6">
            
            <div class="flex justify-between items-center mb-6 pb-2">
                <h2 class="text-2xl font-bold text-gray-800">
                    <i class="fas fa-tachometer-alt mr-2 text-[#7a3229]"></i> Dashboard Overview
                </h2>
            </div>

            <!-- THE UNIFIED CONTAINER (Stats + Table nested together) -->
            <div class="bg-white rounded-lg shadow-md p-6 mb-8 border border-gray-100">
                
                <!-- NESTED Stats Overview Grid -->
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                    <!-- Pending Bookings -->
                    <div class="bg-gray-50 rounded-lg shadow-sm p-5 border border-gray-200 hover:border-blue-300 transition">
                        <div class="flex items-center">
                            <div class="p-3 rounded-full bg-blue-100 text-blue-600 mr-4">
                                <i class="fas fa-clock text-xl"></i>
                            </div>
                            <div>
                                <h3 class="text-2xl font-bold text-gray-800" id="waiting-count">0</h3>
                                <p class="text-gray-600 text-xs font-semibold uppercase tracking-wide mt-1">Pending Bookings</p>
                            </div>
                        </div>
                    </div>

                    <!-- Accepted Bookings -->
                    <div class="bg-gray-50 rounded-lg shadow-sm p-5 border border-gray-200 hover:border-green-300 transition">
                        <div class="flex items-center">
                            <div class="p-3 rounded-full bg-green-100 text-green-600 mr-4">
                                <i class="fas fa-user-check text-xl"></i>
                            </div>
                            <div>
                                <h3 class="text-2xl font-bold text-gray-800" id="serving-count">0</h3>
                                <p class="text-gray-600 text-xs font-semibold uppercase tracking-wide mt-1">Accepted Bookings</p>
                            </div>
                        </div>
                    </div>

                    <!-- On Tours -->
                    <div class="bg-gray-50 rounded-lg shadow-sm p-5 border border-gray-200 hover:border-purple-300 transition">
                        <div class="flex items-center">
                            <div class="p-3 rounded-full bg-purple-100 text-purple-600 mr-4">
                                <i class="fas fa-horse text-xl"></i>
                            </div>
                            <div>
                                <h3 class="text-2xl font-bold text-gray-800" id="completed-count">0</h3>
                                <p class="text-gray-600 text-xs font-semibold uppercase tracking-wide mt-1">On Tours</p>
                            </div>
                        </div>
                    </div>
                
                    <!-- Completed Tours -->
                    <div class="bg-gray-50 rounded-lg shadow-sm p-5 border border-gray-200 hover:border-red-300 transition">
                        <div class="flex items-center">
                            <div class="p-3 rounded-full bg-red-100 text-red-600 mr-4">
                                <i class="fas fa-check-circle text-xl"></i>
                            </div>
                            <div>
                                <h3 class="text-2xl font-bold text-gray-800" id="today-count">0</h3>
                                <p class="text-gray-600 text-xs font-semibold uppercase tracking-wide mt-1">Completed Tours</p>
                            </div>
                        </div>
                    </div>
                </div>
                
                <!-- NESTED Table -->
                
                <div class="flex justify-between items-center mb-4 border-b pb-3">
                        <h3 class="text-lg font-bold text-gray-800">
                            <i class="fas fa-receipt mr-2 text-[#7a3229]"></i>Recent Bookings
                        </h3>
                        
                        <div class="flex items-center space-x-2">
                            <label for="booking-filter" class="text-sm font-semibold text-gray-600">Filter by Type:</label>
                            <select id="booking-filter" onchange="filterBookings(this.value)" class="text-sm border border-gray-300 rounded-md px-3 py-1.5 outline-none focus:ring-2 focus:ring-[#7a3229] text-gray-700 shadow-sm bg-gray-50">
                                <option value="all">All Bookings</option>
                                <option value="attraction">Attractions Only</option>
                                <option value="package">Packages Only</option>
                            </select>
                        </div>
                    </div>

                    <div class="overflow-x-auto">
                        <table class="w-full table-auto border-collapse whitespace-nowrap">
                            <thead>
                                <tr class="bg-gray-100 text-left text-gray-600 uppercase text-[10px] tracking-wider">
                                    <th class="px-3 py-3 font-bold border-b">Booking ID</th>
                                    <th class="px-3 py-3 font-bold border-b">Type</th>
                                    <th class="px-3 py-3 font-bold border-b text-center">Adults and Seniors</th>
                                    <th class="px-3 py-3 font-bold border-b text-center">Children</th>
                                    <th class="px-3 py-3 font-bold border-b text-center">Infants</th>
                                    <th class="px-3 py-3 font-bold border-b">Contact Person</th>
                                    <th class="px-3 py-3 font-bold border-b">Email Address</th>
                                    <th class="px-3 py-3 font-bold border-b">Phone Number</th>
                                    <th class="px-3 py-3 font-bold border-b">Schedule</th>
                                    <th class="px-3 py-3 font-bold border-b text-center">Status</th>
                                    <th class="px-3 py-3 font-bold border-b">Vehicle Name</th>
                                    <th class="px-3 py-3 font-bold border-b text-center">Capacity</th>
                                    <th class="px-3 py-3 font-bold border-b">Tour Guide</th>
                                    <th class="px-3 py-3 font-bold border-b">Email Address</th>
                                </tr>
                            </thead>
                            <tbody id="dashboard-table-body" class="text-gray-700 divide-y divide-gray-100">
                                <tr>
                                    <td colspan="14" class="px-4 py-8 text-center text-gray-500 font-medium">
                                        <i class="fas fa-spinner fa-spin mr-2"></i>Loading live data...
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                </div> 
            </main>
        
        <!-- FOOTER: Outside of <main> but securely inside the Flex Wrapper -->
        <!-- This locks the footer permanently to the bottom of the screen! -->
        <footer class="bg-white border-t border-gray-200 w-full text-center py-4 text-sm text-gray-500 shrink-0">
            &copy; 2026 RENTramuros. All rights reserved.
        </footer>

    </div>

    <!-- JavaScript to load the top stats -->
    <script src="js/history_management.js"></script>
    <script src="js/status_stats.js"></script>

    <script>
        // Keep your clock running on this page too!
        function updateClock() {
            const timeEl = document.getElementById('current-time');
            if(timeEl) {
                const now = new Date();
                timeEl.textContent = now.toLocaleTimeString('en-PH', { hour: '2-digit', minute: '2-digit', second: '2-digit' });
            }
        }
        updateClock();
        setInterval(updateClock, 1000);
    </script>
</body>
</html>