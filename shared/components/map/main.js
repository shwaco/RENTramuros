const map = L.map('map').setView([14.590370542944203, 120.97540387935484], 17);

L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', {
    maxZoom: 19,
    attribution: '&copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a>'
}).addTo(map);  

const customIcon = L.icon({
    iconUrl: "https://cdn-icons-png.flaticon.com/512/18911/18911242.png",
    iconSize: [60, 60]
});

var marker1 = L.marker([14.594368660501456, 120.9704119965505], { icon: customIcon }).addTo(map)
    .bindPopup(`
        <div style="text-align: center;">
            <img src="../../asset/img/fort_santiago/pic1_main.jpg" style="border-radius: 8px; margin-bottom: 5px;">
            <h3>Fort Santiago</h3>
        </div>
    `, { autoClose: false, closeButton: false })    
    .on('mouseover', function (e) {
        this.openPopup();
    })
    .on('mouseout', function (e) {
        this.closePopup(); 
    })
    .on('click', function (e) {
    
    map.flyTo(e.latlng, 18, {
        animate:true,
        duration: 1.0
    });

        document.getElementById('panel-title').innerText = "Fort Santiago";
        document.getElementById('panel-img').src = "../../asset/img/fort_santiago/pic2_mini_one.jpg";
        document.getElementById('panel-text').innerText = "Known as one of the most important historical landmarks in the Philippines, Fort Santiago is a monumental fortress known for its beautiful gardens, Spanish stone gates, and powerful stories from centuries past in the colonial era. As you walk through the fort, explore the exhibits that shed light on history, and walk the same paths taken by the national hero: Jose Rizal.";
        document.getElementById('panel-btn').href = "#";
        
        document.getElementById('panel-operating-hours-details').innerHTML = `
            Monday: 8:00AM - 11:00PM <br>
            Tuesday: 8:00AM - 11:00PM <br>
            Wednesday: 8:00AM - 11:00PM <br>
            Thursday: 8:00AM - 11:00PM <br>
            Friday: 8:00AM - 11:00PM <br>
            Saturday: 8:00AM - 11:00PM <br>
            Sunday: 8:00AM - 11:00PM
            `;
        document.getElementById('side-panel').classList.add('open');
    });
var marker2 = L.marker([14.591924681662595, 120.97341080739596], { icon: customIcon }).addTo(map)
    .bindPopup(`
        <div style="text-align: center;">
            <img src="../../asset/img/minor_basilica/pic1_main.jpg" style="border-radius: 10px; margin-bottom: 10px;">
            <h3>Minor Basilica</h3>
        </div>
    `, { autoClose: false, closeButton: false })
    .on('mouseover', function (e) {
        this.openPopup();
    })
    .on('mouseout', function (e) {
        this.closePopup(); 
    })
    .on('click', function (e) {

        map.flyTo(e.latlng, 18, {
        animate:true,
        duration: 1.0
    });
        document.getElementById('panel-title').innerText = "Minor Basilica";
        document.getElementById('panel-img').src = "../../asset/img/minor_basilica/pic2_mini_one.jpg";
        document.getElementById('panel-text').innerText = "One of the most iconic churches in the Philippines, this grand cathedral with its Romanesque architecture, stained glass windows, and lively interior make it the main must-visit spiritual and cultural landmark to visit.  ";
        document.getElementById('panel-btn').href = "#";

        document.getElementById('panel-operating-hours-details').innerHTML = `
            Monday: CLOSED <br>
            Tuesday: 8:00AM - 4:30PM <br>
            Wednesday: 8:00AM - 4:30PM <br>
            Thursday: 8:00AM - 4:30PM <br>
            Friday: 8:00AM - 4:30PM <br>
            Saturday: 8:00AM - 4:30PM<br>
            Sunday: 8:00AM - 11:30PM
            `;
        document.getElementById('side-panel').classList.add('open');
    });
var marker3 = L.marker([14.589338241759469, 120.97513033287642], { icon: customIcon }).addTo(map)
    .bindPopup(`
        <div style="text-align: center;">
            <img src="../../asset/img/san_agustin_church/pic1_main.jpg" style="border-radius: 8px; margin-bottom: 10px;">
            <h3>San Agustin Church</h3>
        </div>
    `, { autoClose: false, closeButton: false })
    .on('mouseover', function (e) {
        this.openPopup();
    })
    .on('mouseout', function (e) {
        this.closePopup(); 
    })
    .on('click', function (e) {

        map.flyTo(e.latlng, 18, {
        animate:true,
        duration: 1.0
    });
        document.getElementById('panel-title').innerText = "San Agustin Church";
        document.getElementById('panel-img').src = "../../asset/img/san_agustin_church/pic2_mini_one.jpg";
        document.getElementById('panel-text').innerText = "The oldest stone church in the Philippines and a UNESCO World Heritage Site, it is famous for its Baroque architecture, and vividly painted ceilings. Inside the convent museum, visitors can explore centuries of history, artifacts, stories, and religious arts from the colonial period.";
        document.getElementById('panel-btn').href = "#";
        
        document.getElementById('panel-operating-hours-details').innerHTML = `
                Monday: CLOSED <br>
                Tuesday: 8:00AM - 5:00PM <br>
                Wednesday: 8:00AM - 5:00PM <br>
                Thursday: 8:00AM - 5:00PM <br>
                Friday: 8:00AM - 5:00PM <br>
                Saturday: 8:00AM - 5:00PM <br>
                Sunday: 8:00AM - 5:00PM
            `;
        document.getElementById('side-panel').classList.add('open');
    });
var marker4 = L.marker([14.589731466055865, 120.97504055051868], { icon: customIcon }).addTo(map)
   .bindPopup(`
        <div style="text-align: center;">
            <img src="../../asset/img/casa_manila/pic1_main.jpg" style="border-radius: 8px; margin-bottom: 10px;">
            <h3>Casa Manila</h3>
        </div>
    `, { autoClose: false, closeButton: false })
    .on('mouseover', function (e) {
        this.openPopup();
    })
    .on('mouseout', function (e) {
        this.closePopup(); 
    })
    .on('click', function (e) {

        map.flyTo(e.latlng, 18, {
        animate:true,
        duration: 1.0
    });
        document.getElementById('panel-title').innerText = "Casa Manila";
        document.getElementById('panel-img').src = "../../asset/img/casa_manila/pic2_mini_one.jpg";
        document.getElementById('panel-text').innerText = "Travel back in time with a beautifully reconstructed Spanish colonial house. Explore and see in-person how wealthy Filipino families lived during the time, featuring antique furniture, elegant decor, and authentic architecture from the colonial era. ";
        document.getElementById('panel-btn').href = "#";
    
        document.getElementById('panel-operating-hours-details').innerHTML = `
            Monday: CLOSED <br>
            Tuesday: 9:00AM - 6:00PM <br>
            Wednesday: 9:00AM - 6:00PM <br>
            Thursday: 9:00AM - 6:00PM <br>
            Friday: 9:00AM - 6:00PM <br>
            Saturday: 9:00AM - 6:00PM <br>
            Sunday: 9:00AM - 6:00PM
            `;
        document.getElementById('side-panel').classList.add('open');
    });
var marker5 = L.marker([14.589130617529605, 120.97498485422233], { icon: customIcon }).addTo(map)
     .bindPopup(`
        <div style="text-align: center;">
            <img src="../../asset/img/san_agustin_museum/pic1_main.jpg" style="border-radius: 8px; margin-bottom: 10px;">
            <h3>San Agustin Convent Museum</h3>
        </div>
    `, { autoClose: false, closeButton: false })
    .on('mouseover', function (e) {
        this.openPopup();
    })
    .on('mouseout', function (e) {
        this.closePopup(); 
    })
    .on('click', function (e) {

        map.flyTo(e.latlng, 18, {
        animate:true,
        duration: 1.0
    });
        document.getElementById('panel-title').innerText = "San Agustin Museum";
        document.getElementById('panel-img').src = "../../asset/img/san_agustin_museum/pic2_mini_one.jpg";
        document.getElementById('panel-text').innerText = "San Agustin Museum opened its doors in 1973 providing access to the attached 16th-century San Agustin Church, a Unesco Heritage site. Features the church's huge collection of religious artifacts, architecture, carvings, furniture, choir books the oldest in the Philippines.";
        document.getElementById('panel-btn').href = "#";

        document.getElementById('panel-operating-hours-details').innerHTML = `
            Monday: 8:00AM - 5:00PM <br>
            Tuesday: 8:00AM - 5:00PM <br>
            Wednesday: 8:00AM - 5:00PM <br>
            Thursday: 8:00AM - 5:00PM <br>
            Friday: 8:00AM - 5:00PM <br>
            Saturday: 8:00AM - 5:00PM <br>
            Sunday: 8:00AM - 5:00PM
            `;
        document.getElementById('side-panel').classList.add('open');
    });
var marker6 = L.marker([14.59469639090222, 120.96959596771443], { icon: customIcon }).addTo(map)
    .bindPopup(`
        <div style="text-align: center;">
            <img src="../../asset/img/rizal_shrine/pic1_main.jpg" style="border-radius: 8px; margin-bottom: 10px;">
            <h3>Rizal Shrine</h3>
        </div>
    `, { autoClose: false, closeButton: false })
    .on('mouseover', function (e) {
        this.openPopup();
    })
    .on('mouseout', function (e) {
        this.closePopup(); 
    })
    .on('click', function (e) {

        map.flyTo(e.latlng, 18, {
        animate:true,
        duration: 1.0
    });
        document.getElementById('panel-title').innerText = "Rizal Shrine";
        document.getElementById('panel-img').src = "../../asset/img/rizal_shrine/pic2_mini_one.jpg";
        document.getElementById('panel-text').innerText = "Also inside Fort Santiago, the Rizal Shrine is dedicated to the life and works of the Philippine’s national hero. Showcasing photographs, manuscripts, and authentic memorabilia, learn about the hero’s final days and his overall impact on Philippine history.";
        document.getElementById('panel-btn').href = "#";
        
        document.getElementById('panel-operating-hours-details').innerHTML = `
            Monday: CLOSED <br>
            Tuesday: 9:00AM - 6:00PM <br>
            Wednesday: 9:00AM - 6:00PM <br>
            Thursday: 9:00AM - 6:00PM <br>
            Friday: 9:00AM - 6:00PM <br>
            Saturday: 9:00AM - 6:00PM <br>
            Sunday: 9:00AM - 6:00PM
             `;
        document.getElementById('side-panel').classList.add('open');
    });
var marker7 = L.marker([14.591682371482037, 120.97248916771434], { icon: customIcon }).addTo(map)
    .bindPopup(`
        <div style="text-align: center;">
            <img src="../../asset/img/palacio_del_gobernador/pic1_main.jpg" style="border-radius: 8px; margin-bottom: 10px;">
            <h3>Palacio Del Gobernador</h3>
        </div>
    `, { autoClose: false, closeButton: false })
    .on('mouseover', function (e) {
        this.openPopup();
    })
    .on('mouseout', function (e) {
        this.closePopup(); 
    })
    .on('click', function (e) {

        map.flyTo(e.latlng, 18, {
        animate:true,
        duration: 1.0
    });
        document.getElementById('panel-title').innerText = "Palacio Del Gobernador";
        document.getElementById('panel-img').src = "../../asset/img/palacio_del_gobernador/pic2_mini_one.jpg";
        document.getElementById('panel-text').innerText = "An important government and historical site in the heart of Intramuros. It was once the residence of Spanish governor-generals, and stands as a symbol of political history during the colonial era. ";
        document.getElementById('panel-btn').href = "#";
        
        document.getElementById('panel-operating-hours-details').innerHTML = `
            Monday: 8:00AM - 5:00PM <br>
            Tuesday: 8:00AM - 5:00PM <br>
            Wednesday: 8:00AM - 5:00PM <br>
            Thursday: 8:00AM - 5:00PM <br>
            Friday: 8:00AM - 5:00PM <br>
            Saturday: CLOSED <br>
            Sunday: CLOSED
            `;
        document.getElementById('side-panel').classList.add('open');
    });
var marker8 = L.marker([14.590052863152057, 120.97345112538642], { icon: customIcon }).addTo(map)
    .bindPopup(`
        <div style="text-align: center;">
            <img src="../../asset/img/museo_de_intramuros/pic1_main.jpg" style="border-radius: 8px; margin-bottom: 10px;">
            <h3>Museo De Intramuros</h3>
        </div>
    `, { autoClose: false, closeButton: false })
    .on('mouseover', function (e) {
        this.openPopup();
    })
    .on('mouseout', function (e) {
        this.closePopup(); 
    })
    .on('click', function (e) {

        map.flyTo(e.latlng, 18, {
        animate:true,
        duration: 1.0
    });
        document.getElementById('panel-title').innerText = "Museo De Intramuros";
        document.getElementById('panel-img').src = "../../asset/img/museo_de_intramuros/pic2_mini_one.jpg";
        document.getElementById('panel-text').innerText = "A must-visit museum, it features beautifully preserved artwork, sculptures, and artifacts from different parts of the country. It showcases the religious and cultural heritage of the Philippines during the colonial era, showing its rich history.";
        document.getElementById('panel-btn').href = "#";
        
        document.getElementById('panel-operating-hours-details').innerHTML = `
            Monday: 9:00AM - 6:00PM <br>
            Tuesday: 9:00AM - 6:00PM <br>
            Wednesday: 9:00AM - 6:00PM <br>
            Thursday: 9:00AM - 6:00PM <br>
            Friday: 9:00AM - 6:00PM <br>
            Saturday: 9:00AM - 6:00PM <br>
            Sunday: CLOSED
             `;
        document.getElementById('side-panel').classList.add('open');
    });
var marker9 = L.marker([14.587684179514117, 120.97698311004227], { icon: customIcon }).addTo(map)
    .bindPopup(`
        <div style="text-align: center;">
            <img src="../../asset/img/silahis_art/pic1_main.jpg" style="border-radius: 8px; margin-bottom: 10px;">
            <h3>Silahi's Art and Artifacts Inc</h3>
        </div>
    `, { autoClose: false, closeButton: false })
    .on('mouseover', function (e) {
        this.openPopup();
    })
    .on('mouseout', function (e) {
        this.closePopup(); 
    })
    .on('click', function (e) {

        map.flyTo(e.latlng, 18, {
        animate:true,
        duration: 1.0
    });
        document.getElementById('panel-title').innerText = "Silahi's Art and Artifacts Inc";
        document.getElementById('panel-img').src = "../../asset/img/silahis_art/pic2_mini_one.jpg";
        document.getElementById('panel-text').innerText = "A treasure trove of Filipino craftsmanship, browse and shop from a wide collection of traditional artworks, handmade souvenirs, antiques, and cultural pieces. It celebrates the creativity and cultural heritage of the Philippines.";
        document.getElementById('panel-btn').href = "#";

        document.getElementById('panel-operating-hours-details').innerHTML = `
            Monday: 10:00AM - 5:00PM <br>
            Tuesday: 10:00AM - 5:00PM <br>
            Wednesday: 10:00AM - 5:00PM <br>
            Thursday: 10:00AM - 5:00PM <br>
            Friday: 10:00AM - 5:00PM <br>
            Saturday: 10:00AM - 5:00PM <br>
            Sunday: 10:00AM - 5:00PM
            `;
        document.getElementById('side-panel').classList.add('open');
    });
var marker10 = L.marker([14.587148079969733, 120.97520281004242], { icon: customIcon }).addTo(map)
    .bindPopup(`
        <div style="text-align: center;">
            <img src="../../asset/img/rizal_bagumbayan/pic1_main.jpg" style="border-radius: 8px; margin-bottom: 10px;">
            <h3>Rizal’s Bagumbayan Light and Sound Museum</h3>
        </div>
    `, { autoClose: false, closeButton: false })
    .on('mouseover', function (e) {
        this.openPopup();
    })
    .on('mouseout', function (e) {
        this.closePopup(); 
    })
    .on('click', function (e) {

        map.flyTo(e.latlng, 18, {
        animate:true,
        duration: 1.0
    });
        document.getElementById('panel-title').innerText = "Rizal's Bagumbayan Light and Sound Museum";
        document.getElementById('panel-img').src = "../../asset/img/rizal_bagumbayan/pic2_mini_one.jpg";
        document.getElementById('panel-text').innerText = "Experience history through an immersive storytelling performance. Learn about the life and martyrdom of Jose Rizal in a stylish and engaging way through dramatic lights, storytelling, and music.";
        document.getElementById('panel-btn').href = "#";
        
        document.getElementById('panel-operating-hours-details').innerHTML = `
            Monday: CLOSED <br>
            Tuesday: 10:00AM - 6:00PM <br>
            Wednesday: 10:00AM - 6:00PM <br>
            Thursday: 10:00AM - 6:00PM <br>
            Friday: 10:00AM - 6:00PM <br>
            Saturday: 10:00AM - 6:00PM <br>
            Sunday: 10:00AM - 6:00PM
        `;
        document.getElementById('side-panel').classList.add('open');
    });
var marker11 = L.marker([14.589662875177895, 120.97510720819086], { icon: customIcon }).addTo(map)
    .bindPopup(`
        <div style="text-align: center;">
            <img src="../../asset/img/barbara/pic1_main.jpg" style="border-radius: 8px; margin-bottom: 10px;">
            <h3>Barbara's Heritage Restaurant</h3>
        </div>
    `, { autoClose: false, closeButton: false })
    .on('mouseover', function (e) {
        this.openPopup();
    })
    .on('mouseout', function (e) {
        this.closePopup(); 
    })
    .on('click', function (e) {

        map.flyTo(e.latlng, 18, {
        animate:true,
        duration: 1.0
    });
        document.getElementById('panel-title').innerText = "Barbara's Heritage Restaurant";
        document.getElementById('panel-img').src = "../../asset/img/barbara/pic2_mini_one.jpg";
        document.getElementById('panel-text').innerText = "Have a taste of history and experience it. Authentic Filipino cuisine in a historic setting, it is a unique dining experience that blends food, music, and heritage. It is known for its traditional dishes and cultural performances. ";
        document.getElementById('panel-btn').href = "#";
        
            document.getElementById('panel-operating-hours-details').innerHTML = `
                Monday: 9:00AM - 9:00PM <br>
                Tuesday: 9:00AM - 9:00PM <br>
                Wednesday: 9:00AM - 9:00PM <br>
                Thursday: 9:00AM - 9:00PM <br>
                Friday: 9:00AM - 9:00PM <br>
                Saturday: 9:00AM - 9:00PM <br>
                Sunday: 9:00AM - 9:00PM
             `;
        document.getElementById('side-panel').classList.add('open');
    });

var marker12 = L.marker([14.589703266065198, 120.97540935422217], { icon: customIcon }).addTo(map)
    .bindPopup(`
        <div style="text-align: center;">
            <img src="../../asset/img/bambike/pic1_main.jpg" style="border-radius: 8px; margin-bottom: 10px;">
            <h3>Bambike Ecotours</h3>
        </div>
    `, { autoClose: false, closeButton: false })
    .on('mouseover', function (e) {
        this.openPopup();
    })
    .on('mouseout', function (e) {
        this.closePopup(); 
    })
    .on('click', function (e) {

        map.flyTo(e.latlng, 18, {
        animate:true,
        duration: 1.0
    });
        document.getElementById('panel-title').innerText = "Bambike Ecotours";
        document.getElementById('panel-img').src = "../../asset/img/bambike/pic2_mini_one.jpg";
        document.getElementById('panel-text').innerText = "Explore the walled city in a fun, unique, and eco-friendly way. Handmade bamboo bikes let visitors ride through the historical walls learning about its rich culture and heritage at your own pace, or through guided tours. It is a fun, relaxing, sustainable, and memorable way to experience and see the charm of Intramuros.";
        document.getElementById('panel-btn').href = "#";
        
        document.getElementById('panel-operating-hours-details').innerHTML = `
            Monday: 9:00AM - 6:00PM <br>
            Tuesday: 9:00AM - 6:00PM <br>
            Wednesday: 9:00AM - 6:00PM <br>
            Thursday: 9:00AM - 6:00PM <br>
            Friday: 9:00AM - 6:00PM <br>
            Saturday: 9:00AM - 6:00PM <br>
            Sunday: 9:00AM - 6:00PM
            `;
        document.getElementById('side-panel').classList.add('open');
    });

var marker13 = L.marker([14.592476271432954, 120.97810108673141], { icon: customIcon }).addTo(map)
    .bindPopup(`
        <div style="text-align: center;">
            <img src="../../asset/img/puerta_del_parian/pic1_main.jpg" style="border-radius: 8px; margin-bottom: 10px;">
            <h3>Puerta Del Parian</h3>
        </div>
    `, { autoClose: false, closeButton: false })
    .on('mouseover', function (e) {
        this.openPopup();
    })
    .on('mouseout', function (e) {
        this.closePopup(); 
    })
    .on('click', function (e) {

        map.flyTo(e.latlng, 18, {
        animate:true,
        duration: 1.0
    });
        document.getElementById('panel-title').innerText = "Puerta Del Parian";
        document.getElementById('panel-img').src = "../../asset/img/puerta_del_parian/pic2_mini_one.jpg";
        document.getElementById('panel-text').innerText = "Puerta Del Parian is among the eight gates that serve as an entrance to the Walled City of Intramuros. The gates were called Puerta, which is the original Spanish word for gate. Hence, the name Puerta del Parian means Gate of the market signifying its crucial role in Philippine history. From the Spanish colonial times to the British occupation is 1762-1764, as this was the place where many enterprising Chinese traded goods. ";
        document.getElementById('panel-btn').href = "#";
        
        document.getElementById('panel-operating-hours-details').innerHTML = `
            Monday: 9:00AM - 6:00PM <br>
            Tuesday: 9:00AM - 6:00PM <br>
            Wednesday: 9:00AM - 6:00PM <br>
            Thursday: 9:00AM - 6:00PM <br>
            Friday: 9:00AM - 6:00PM <br>
            Saturday: 9:00AM - 6:00PM <br>
            Sunday: 9:00AM - 6:00PM
            `;
        document.getElementById('side-panel').classList.add('open');
    });

var marker14 = L.marker([14.586008547062493, 120.9774149118942], { icon: customIcon }).addTo(map)
    .bindPopup(`
        <div style="text-align: center;">
            <img src="../../asset/img/puerta_real_gardens/pic1_main.jpg" style="border-radius: 8px; margin-bottom: 10px;">
            <h3>Puerta Real Gardens</h3>
        </div>
    `, { autoClose: false, closeButton: false })
    .on('mouseover', function (e) {
        this.openPopup();
    })
    .on('mouseout', function (e) {
        this.closePopup(); 
    })
    .on('click', function (e) {

        map.flyTo(e.latlng, 18, {
        animate:true,
        duration: 1.0
    });
        document.getElementById('panel-title').innerText = "Puerta Real Gardens";
        document.getElementById('panel-img').src = "../../asset/img/puerta_real_gardens/pic2_mini_one.jpg";
        document.getElementById('panel-text').innerText = "Puerta Real Gardens is a beautiful garden located within the Walled City of Intramuros. It offers a peaceful retreat for visitors to relax and enjoy the natural surroundings while learning about the history and culture of the area. ";
        document.getElementById('panel-btn').href = "#";
        
        document.getElementById('panel-operating-hours-details').innerHTML = `
            Monday: CLOSED <br>
            Tuesday: 9:00AM - 5:00PM <br>
            Wednesday: 9:00AM - 5:00PM <br>
            Thursday: 9:00AM - 5:00PM <br>
            Friday: 9:00AM - 5:00PM <br>
            Saturday: 9:00AM - 5:00PM <br>
            Sunday: 9:00AM - 5:00PM
            `;
        document.getElementById('side-panel').classList.add('open');
    });

var marker15 = L.marker([14.59026652873885, 120.97346745792579], { icon: customIcon }).addTo(map)
    .bindPopup(`
        <div style="text-align: center;">
            <img src="../../asset/img/centro_de_turismo/pic1_main.jpg" style="border-radius: 8px; margin-bottom: 10px;">
            <h3>Centro de Turismo Intramuros</h3>
        </div>
    `, { autoClose: false, closeButton: false })
    .on('mouseover', function (e) {
        this.openPopup();
    })
    .on('mouseout', function (e) {
        this.closePopup(); 
    })
    .on('click', function (e) {

        map.flyTo(e.latlng, 18, {
        animate:true,
        duration: 1.0
    });
        document.getElementById('panel-title').innerText = "Centro de Turismo Intramuros";
        document.getElementById('panel-img').src = "../../asset/img/centro_de_turismo/pic2_mini_one.jpg";
        document.getElementById('panel-text').innerText = "Centro de Turismo Intramuros is a visitor center that provides information and services for tourists exploring the historic district of Intramuros. It serves as a hub for cultural and historical activities, offering exhibits and interactive displays about the rich heritage of the area. ";
        document.getElementById('panel-btn').href = "#";
        
        document.getElementById('panel-operating-hours-details').innerHTML = `
            Monday: CLOSED <br>
            Tuesday: 9:00AM - 6:00PM <br>
            Wednesday: 9:00AM - 6:00PM <br>
            Thursday: 9:00AM - 6:00PM <br>
            Friday: 9:00AM - 6:00PM <br>
            Saturday: 9:00AM - 6:00PM <br>
            Sunday: 9:00AM - 6:00PM
            `;
        document.getElementById('side-panel').classList.add('open');
    });

const searchMap = {
    "Fort Santiago": marker1,
    "Minor Basilica": marker2,
    "San Agustin Church": marker3,
    "Casa Manila": marker4,
    "San Agustin Convent Museum": marker5,
    "Rizal Shrine": marker6,
    "Palacio Del Gobernador": marker7,
    "Museo De Intramuros": marker8,
    "Silahi's Art And Artifacts Inc": marker9,
    "Rizal's Bagumbayan Light and Sound Museum": marker10,
    "Barbara's Heritage Restaurant": marker11,
    "Bambike Ecotours": marker12,
    "Puerta Del Parian": marker13,
    "Puerta Real Gardens": marker14,
    "Centro de Turismo Intramuros": marker15
};

document.getElementById('search-btn').addEventListener('click', function() {

    let query = document.getElementById('search-input').value.toLowerCase();
    let targetMarker = null;

    for (let key in searchMap) { searchMap[key].closeTooltip(); }

    for (let locationName in searchMap) {
        if (locationName.toLowerCase() === query) {
            targetMarker = searchMap[locationName];
            break;
        }
    }
    
    if (targetMarker) {
        map.closePopup();

        hideAllMarkers();

        targetMarker.addTo(map);

        targetMarker.fire('click', { latlng: targetMarker.getLatLng() }); 

        setTimeout(function() {
            targetMarker.openPopup();
        }, 500);
        
    } else {
        alert("Location not found.");
    }
});

document.getElementById('search-input').addEventListener('keypress', function(event) {
    if (event.key === "Enter") {
        event.preventDefault(); 
        document.getElementById('search-btn').click();
    }
});

window.addEventListener('load', function() {
    document.getElementById('search-input').focus();
});

// Filtration system dito ---

function hideAllMarkers() {
    for (let key in searchMap) {
        map.removeLayer(searchMap[key]); 
    }
}

const museumMarkers = [marker8, marker5, marker9, marker10, marker15];

document.getElementById('museums-buttons').addEventListener('click', function() {

    map.closePopup();
    for (let key in searchMap) { 
        searchMap[key].closeTooltip(); 
    }

    hideAllMarkers();

    museumMarkers.forEach(function(marker) {
        marker.addTo(map);
    });

    const museumGroup = new L.featureGroup(museumMarkers);
    map.flyToBounds(museumGroup.getBounds(), { padding: [80, 80], maxZoom: 17, duration: 1.0});

    setTimeout(function() {
        museumMarkers.forEach(function(marker) {
            marker.openTooltip();
        });
    }, 1500);
});



const foodplacesMarkers = [marker4, marker11];

document.getElementById('foodplaces-buttons').addEventListener('click', function() {
    
    map.closePopup();
    for (let key in searchMap) { 
        searchMap[key].closeTooltip(); 
    }

    hideAllMarkers();

    foodplacesMarkers.forEach(function(marker) {
        marker.addTo(map);
    });

    const foodGroup = new L.featureGroup(foodplacesMarkers);
    map.flyToBounds(foodGroup.getBounds(), { padding: [80,80], maxZoom: 20, duration: 1.0});

    setTimeout(function() {
        foodplacesMarkers.forEach(function(marker) {
           marker.openTooltip();
        });
    }, 1500);
});

const churchesMarkers = [marker2, marker3];

document.getElementById('church-buttons').addEventListener('click', function() {

    map.closePopup();
    for (let key in searchMap) {
        searchMap[key].closeTooltip();
    }

    hideAllMarkers();

    churchesMarkers.forEach(function(marker) {
        marker.addTo(map);
    });

    const churchGroup = new L.featureGroup(churchesMarkers);
    map.flyToBounds(churchGroup.getBounds(), { padding: [80,80], maxZoom: 20, duration: 1.0});

    setTimeout(function() {
        churchesMarkers.forEach(function(marker) {
           marker.openTooltip();
        });
    }, 1500);
});

const LandmarkMarkers = [marker1, marker6, marker7, marker13, marker14];

document.getElementById('landmarks-buttons').addEventListener('click', function() {

    map.closePopup();
    for (let key in searchMap) {
        searchMap[key].closeTooltip();
    }

    hideAllMarkers();

    LandmarkMarkers.forEach(function(marker) {
        marker.addTo(map);
    });

    const landmarkGroup = new L.featureGroup(LandmarkMarkers);
    map.flyToBounds(landmarkGroup.getBounds(), { padding: [80,80], maxzoom: 16, duration: 1.0});

    setTimeout(function() {
        LandmarkMarkers.forEach(function(marker) {
            marker.openTooltip();
        });
    }, 1500);
});

const RidesMarkers = [marker12];

document.getElementById('rides-buttons').addEventListener('click', function() {

    map.closePopup();
    for (let key in searchMap) {
        searchMap[key].closeTooltip();
    }

    hideAllMarkers();

    RidesMarkers.forEach(function(marker) {
        marker.addTo(map);
    });

    const ridesGroup = new L.featureGroup(RidesMarkers);
    map.flyToBounds(ridesGroup.getBounds(), { padding: [80,80], maxzoom: 19, duration: 1.0});

    setTimeout(function() {
        RidesMarkers.forEach(function(marker) {
            marker.openTooltip();
        });
    }, 1500);
});

document.getElementById('show-all-btn').addEventListener('click', function() {
    map.closePopup();
    
    for (let key in searchMap) {
        searchMap[key].addTo(map);
        searchMap[key].closeTooltip(); 
    }

    map.flyTo([14.590370542944203, 120.97540387935484], 16, {
        animate: true,
        duration: 1.0
    });
});

// polisher --

map.on('click', function() {
    for (let pinName in searchMap) {
        searchMap[pinName].isClicked = false;
        searchMap[pinName].closePopup();
        searchMap[pinName].closeTooltip();
    }
});

for (let locationName in searchMap) {
    let currentMarker = searchMap[locationName];

    currentMarker.bindTooltip(locationName, { permanent: true, direction: 'top', offset: [0, -35], className: 'mini-label' });
    currentMarker.closeTooltip();
    
    currentMarker.on('click', function() {
        let clickedMarker = this;

        for (let pinName in searchMap) {
            searchMap[pinName].closeTooltip();

            if (searchMap[pinName] !== clickedMarker) {
                searchMap[pinName].closePopup();
        }
    }
    
    clickedMarker.openPopup();
    });
}

// --- INTERACTIVE MAP BOOKING REDIRECT ---
document.addEventListener('DOMContentLoaded', () => {
    const bookNowBtn = document.getElementById('panel-btn');
    
    if (bookNowBtn) {
        // Alisin ang target="_blank" para hindi magbukas ng bagong tab
        bookNowBtn.removeAttribute('target');
        
        bookNowBtn.addEventListener('click', (e) => {
            e.preventDefault(); // Pigilan ang default na '#' link

            if (window.IS_LOGGED_IN) {
                // Kung naka-login, diretso sa Tour Booking page
                // Note: ../tour_page/ ang path dahil galing tayo sa pages/landing_page/
                window.location.href = '../tour_page/tours_latest.php';
            } else {
                // Kung hindi naka-login, sa Login page
                window.location.href = '../login_page/login.php';
            }
        });
    }
});