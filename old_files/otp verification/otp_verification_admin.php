...
if(!isset($_SESSION['temp_email'])) {
    header("location: auth/sign up/signup_admin.php");
    exit();
}
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    $sql = "SELECT * FROM `admins` WHERE email='$email' AND otp='$entered_otp'";
    $result = mysqli_query($con, $sql);

    if($result && mysqli_num_rows($result) > 0) {
        // The OTP is correct! Update the database
        $update_sql = "UPDATE `admins`SET is_verified=1, otp=NULL WHERE email='$email'";
...
<html lang="en">
  ...
  <?php
    if($success) {
        ...
        <strong>Success!</strong> Your account is verified. You can now <a href="auth/log in/login_admin.php" class="alert-link">Log In</a>.
  ...
  ?>
    ...
    <div class="container mt-5" style="max-width: 500px;">
        ...
        <form action="auth/otp verification/otp_verification_admin.php" method="POST">
          <div class="mb-3">
